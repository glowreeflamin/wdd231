...
const placeDetails = document.querySelector("#place-details"); // Select the place details container
const url = "data/places.json"; // URL to the JSON file
...
function displayPlaces(places) {
  placeContainer.innerHTML = ""; // Clear the container before displaying new data
  ...
  places.slice(0, 4).forEach((place) => {
    ...
    placeItem.addEventListener("click", () => {
      ...
      document.querySelector(".close").addEventListener("click", () => {
          placeDetails.close(); // Clear the details when the close button is clicked
      });
  
      placeDetails.showModal(); // Show the modal with place details

    });

    placeContainer.appendChild(placeItem); // Append the div to the container
});
